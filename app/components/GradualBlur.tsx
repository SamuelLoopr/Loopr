...
// React Bits GradualBlur, converted to TypeScript to match the rest of
// app/components. Layers stacked backdrop-filter divs behind a mask gradient to
// fade content out toward one edge — target="parent" pins it inside the nearest
// positioned ancestor, target="page" fixes it to the viewport.
...
type BlurConfig = Required<Pick<GradualBlurProps, 'position' | 'strength' | 'height' | 'divCount' |
  'exponential' | 'zIndex' | 'animated' | 'duration' | 'easing' | 'opacity' | 'curve' |
  'responsive' | 'target' | 'className' | 'style'>> & GradualBlurProps & Record<string, unknown>
...
const mergeConfigs = (...configs: Record<string, unknown>[]): BlurConfig =>
  configs.reduce((acc, c) => ({ ...acc, ...c }), {}) as BlurConfig
...
function GradualBlur(props: GradualBlurProps) {
  ...
  const config = useMemo(() => {
    const presetConfig = (props.preset && (PRESETS as Record<string, Record<string, unknown>>)[props.preset]) || {}
